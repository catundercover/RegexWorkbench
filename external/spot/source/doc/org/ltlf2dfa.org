...
* Support for Quantified LTLf

This tool can translate LTLf formulas that are quantified at the top-level.

Compare:

#+NAME: ltlf2dfa8a
#+BEGIN_SRC sh :exports code
ltlf2dfa --keep-names '((GFu)->F(i<->o))&((GF!u)->F(i|o))' --dot=.A
#+END_SRC

#+BEGIN_SRC dot :file ltlf2dfa8a.svg :var txt=ltlf2dfa8a :exports results
  $txt
#+END_SRC
...
#+NAME: ltlf2dfa8b
#+BEGIN_SRC sh :exports code
ltlf2dfa --keep-names '\forall u: ((GFu)->F(i<->o))&((GF!u)->F(i|o))' --dot=.A
#+END_SRC

#+BEGIN_SRC dot :file ltlf2dfa8b.svg :var txt=ltlf2dfa8b :exports results
  $txt
#+END_SRC
...
The following example requests that accepted words have even length
(=even= implies =X(!even)= so the automaton could stop, but =!even=
implies =X[!]even= so the automaton may not stop), and that =a= should
hold at each even step (=a= is free on odd steps).  The variable
=even= is used to keep track of the parity of the steps during the
construction, but it does not appear in the resulting automaton
because it has been quantified away.

#+NAME: ltlf2dfa8c
#+BEGIN_SRC sh :exports code
ltlf2dfa --keep-names '\exists even: !even & G(even <-> X(!even)) & G(even -> a)'  --dot=.A
#+END_SRC

#+BEGIN_SRC dot :file ltlf2dfa8c.svg :var txt=ltlf2dfa8c :exports results
  $txt
#+END_SRC
...
Quantifiers may also be nested, as long as they all appear at the
top-level.  For instance here is a convoluted way to build a DFA that
accepts words where =r= holds exactly once.

#+NAME: ltlf2dfa8d
#+BEGIN_SRC sh :exports code
ltlf2dfa --keep-names '\exists p: \forall q: F(p) & (F(p&q)->G(p->q)) & G(r<->p)' --dot=.A
#+END_SRC

#+BEGIN_SRC dot :file ltlf2dfa8d.svg :var txt=ltlf2dfa8d :exports results
  $txt
#+END_SRC
...
- With =--translation=direct=, the entire formula is translated
  ...
  translation.  This is the only approach that supports /quantified/
  LTLf formulas.
...
By default, =--translation=auto= will use the compositional approach
for unquantified LTLf formulas, and the direct approach for quantified
LTLf formulas.
