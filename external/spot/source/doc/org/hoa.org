...
typical acceptance conditions like Büchi, generalized-Büchi,
...
of the tools distributed by Spot, and it also looks at some lower-level,
...
- Multiple (or missing) initial states are emulated.
  ...
  - If one of the original initial states has no incoming edge, it can
    ...
    optimization, if the other initial state also does not have any
...
each state belong to the same sets:
...
=Acceptance:= line).  However, the HOA printer has no idea of
...
construct a new automaton, those properties will not be carried over
