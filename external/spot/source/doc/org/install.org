...
Past releases can be found [[https://www.lre.epita.fr/dload/spot/][in the same directory]].  If you are
...
Spot requires a C++20-compliant compiler.  =g++= 10.0 or later, as well
as =clang++= 10.0 or later should work.

Spot expects a complete installation of Python (version 3.6 or later).
Especially, Python's header files should be installed (the package to
...
Trixie (a.k.a. Debian stable) while packages for development are built
for Sid (a.k.a. Debian unstable).

Here is how to install the stable packages (as root):
...
To install the unstable packages, simply replace =stable/= by =unstable/=.
...
9E74 44F2 A84A=, if you want to verify it (run =gpg --show-keys
/etc/apt/keyrings/lre-epita.gpg=).  This key should be valid
until 2032.

The package =spot= contains the [[file:tools.org][command-line tools]]. =libspot-dev=
contains the header files if you plan to use Spot in a C++20
...
[[https://download.opensuse.org/repositories/home:/adl/][This OBS directory]] contains builds of the latest release of Spot for
several Debian-based distributions, e.g., Ubuntu or Raspbian
distributions.  It may be lagging behind the latest release of Spot by
a few days, as the update is not entirely automatic.
...
#+BEGIN_SRC sh
  wget -q -O - 'https://build.opensuse.org/projects/home:adl/signing_keys/download?kind=gpg' |
      gpg --dearmor -o /etc/apt/keyrings/home-adl-obs.gpg
  echo 'deb [signed-by=/etc/apt/keyrings/home-adl-obs.gpg] https://download.opensuse.org/repositories/home:/adl/xUbuntu_25.04/ ./' \
