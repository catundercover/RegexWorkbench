...
=beginline.column-endline.column= specifying the position of that
...
=autfilt=.  This is a bit like adding =cat %H>%O= as another tool: it
...
examples would not exist if the language was really preserved by the
tool.
