...
* Overview of the changes
  ...
  1. [[#cpp20][Spot now compiles using the C++20 standard]].  Compliant compilers
...
* Upgrading to C++20
  ...
  Because Spot now relies on C++20 features, programs that use Spot
  should at least be compiled using this version (or a later one) of
  the language.

  This is usually done by passing the option =-std=c++20= to =g++= or
  =clang++=.

  Upgrading from C++98, C++03, C++11, C++14, or C++17 to C++20 should be
...
Because the above loop is quite common we also have some syntactic
...
#  LocalWords:  neverparse saba tgbaalgos dualize tgbaparse
