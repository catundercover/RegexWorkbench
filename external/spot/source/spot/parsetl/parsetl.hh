...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // aplist
      char dummy4[sizeof (std::vector<const spot::fnode*>)];

      // "number for square bracket operator"
      // "SVA delay operator"
      // sqbkt_num
      char dummy5[sizeof (unsigned)];
    };
    ...
    OP_EXISTS = 284,               // "exists operator"
    OP_FORALL = 285,               // "forall operator"
    COMMA = 286,                   // ","
    OP_NOT = 287,                  // "not operator"
    OP_XREP = 288,                 // "X[.] operator"
    OP_FREP = 289,                 // "F[.] operator"
    OP_GREP = 290,                 // "G[.] operator"
    OP_STAR = 291,                 // "star operator"
    OP_BSTAR = 292,                // "bracket star operator"
    OP_BFSTAR = 293,               // "bracket fusion-star operator"
    OP_PLUS = 294,                 // "plus operator"
    OP_FPLUS = 295,                // "fusion-plus operator"
    OP_STAR_OPEN = 296,            // "opening bracket for star operator"
    OP_FSTAR_OPEN = 297,           // "opening bracket for fusion-star operator"
    OP_EQUAL_OPEN = 298,           // "opening bracket for equal operator"
    OP_GOTO_OPEN = 299,            // "opening bracket for goto operator"
    OP_SQBKT_CLOSE = 300,          // "closing bracket"
    OP_SQBKT_STRONG_CLOSE = 301,   // "closing !]"
    OP_SQBKT_NUM = 302,            // "number for square bracket operator"
    OP_UNBOUNDED = 303,            // "unbounded mark"
    OP_SQBKT_SEP = 304,            // "separator for square bracket operator"
    OP_UCONCAT = 305,              // "universal concat operator"
    OP_ECONCAT = 306,              // "existential concat operator"
    OP_UCONCAT_NONO = 307,         // "universal non-overlapping concat operator"
    OP_ECONCAT_NONO = 308,         // "existential non-overlapping concat operator"
    OP_FIRST_MATCH = 309,          // "first_match"
    ATOMIC_PROP = 310,             // "atomic proposition"
    OP_CONCAT = 311,               // "concat operator"
    OP_FUSION = 312,               // ":"
    CONST_TRUE = 313,              // "constant true"
    CONST_FALSE = 314,             // "constant false"
    END_OF_INPUT = 315,            // "end of formula"
    OP_POST_NEG = 316,             // "negative suffix"
    OP_POST_POS = 317,             // "positive suffix"
    OP_DELAY_N = 318,              // "SVA delay operator"
    OP_DELAY_OPEN = 319,           // "opening bracket for SVA delay operator"
    OP_DELAY_PLUS = 320,           // "##[+] operator"
    OP_DELAY_STAR = 321            // "##[*] operator"
      };
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_OP_EXISTS = 29,                        // "exists operator"
        S_OP_FORALL = 30,                        // "forall operator"
        S_COMMA = 31,                            // ","
        S_OP_NOT = 32,                           // "not operator"
        S_OP_XREP = 33,                          // "X[.] operator"
        S_OP_FREP = 34,                          // "F[.] operator"
        S_OP_GREP = 35,                          // "G[.] operator"
        S_OP_STAR = 36,                          // "star operator"
        S_OP_BSTAR = 37,                         // "bracket star operator"
        S_OP_BFSTAR = 38,                        // "bracket fusion-star operator"
        S_OP_PLUS = 39,                          // "plus operator"
        S_OP_FPLUS = 40,                         // "fusion-plus operator"
        S_OP_STAR_OPEN = 41,                     // "opening bracket for star operator"
        S_OP_FSTAR_OPEN = 42,                    // "opening bracket for fusion-star operator"
        S_OP_EQUAL_OPEN = 43,                    // "opening bracket for equal operator"
        S_OP_GOTO_OPEN = 44,                     // "opening bracket for goto operator"
        S_OP_SQBKT_CLOSE = 45,                   // "closing bracket"
        S_OP_SQBKT_STRONG_CLOSE = 46,            // "closing !]"
        S_OP_SQBKT_NUM = 47,                     // "number for square bracket operator"
        S_OP_UNBOUNDED = 48,                     // "unbounded mark"
        S_OP_SQBKT_SEP = 49,                     // "separator for square bracket operator"
        S_OP_UCONCAT = 50,                       // "universal concat operator"
        S_OP_ECONCAT = 51,                       // "existential concat operator"
        S_OP_UCONCAT_NONO = 52,                  // "universal non-overlapping concat operator"
        S_OP_ECONCAT_NONO = 53,                  // "existential non-overlapping concat operator"
        S_OP_FIRST_MATCH = 54,                   // "first_match"
        S_ATOMIC_PROP = 55,                      // "atomic proposition"
        S_OP_CONCAT = 56,                        // "concat operator"
        S_OP_FUSION = 57,                        // ":"
        S_CONST_TRUE = 58,                       // "constant true"
        S_CONST_FALSE = 59,                      // "constant false"
        S_END_OF_INPUT = 60,                     // "end of formula"
        S_OP_POST_NEG = 61,                      // "negative suffix"
        S_OP_POST_POS = 62,                      // "positive suffix"
        S_OP_DELAY_N = 63,                       // "SVA delay operator"
        S_OP_DELAY_OPEN = 64,                    // "opening bracket for SVA delay operator"
        S_OP_DELAY_PLUS = 65,                    // "##[+] operator"
        S_OP_DELAY_STAR = 66,                    // "##[*] operator"
        S_67_ = 67,                              // '!'
        S_68_ = 68,                              // '&'
        S_69_ = 69,                              // '|'
        S_70_ = 70,                              // '^'
        ...
        S_result = 84,                           // result
        S_emptyinput = 85,                       // emptyinput
        S_enderror = 86,                         // enderror
        S_OP_SQBKT_SEP_unbounded = 87,           // OP_SQBKT_SEP_unbounded
        S_OP_SQBKT_SEP_opt = 88,                 // OP_SQBKT_SEP_opt
        S_error_opt = 89,                        // error_opt
        S_sqbkt_num = 90,                        // sqbkt_num
        S_sqbracketargs = 91,                    // sqbracketargs
        S_gotoargs = 92,                         // gotoargs
        S_kleen_star = 93,                       // kleen_star
        S_starargs = 94,                         // starargs
        S_fstarargs = 95,                        // fstarargs
        S_equalargs = 96,                        // equalargs
        S_delayargs = 97,                        // delayargs
        S_atomprop = 98,                         // atomprop
        S_booleanatom = 99,                      // booleanatom
        S_sere = 100,                            // sere
        S_bracedsere = 101,                      // bracedsere
        S_parenthesedsubformula = 102,           // parenthesedsubformula
        S_boolformula = 103,                     // boolformula
        S_aplist = 104,                          // aplist
        S_maybequantifiedformula = 105,          // maybequantifiedformula
        S_exists_or_forall = 106,                // exists_or_forall
        S_quantifiedformula = 107,               // quantifiedformula
        S_subformula = 108,                      // subformula
        S_lbtformula = 109                       // lbtformula
      };
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_aplist: // aplist
        value.move< std::vector<const spot::fnode*> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::vector<const spot::fnode*>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::vector<const spot::fnode*>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#line 428 "parsetl.yy"
  ...
  for (unsigned i = 0; i < yysym.value.template as < std::vector<const spot::fnode*> > ().size(); ++i)
    yysym.value.template as < std::vector<const spot::fnode*> > ()[i]->destroy();
}
#line 1011 "parsetl.hh"
        break;
...
switch (yykind)
    {
      ...
      case symbol_kind::S_aplist: // aplist
        value.template destroy< std::vector<const spot::fnode*> > ();
        break;
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_OP_EXISTS (location_type l)
      {
        return symbol_type (token::OP_EXISTS, std::move (l));
      }
#else
      static
      symbol_type
      make_OP_EXISTS (const location_type& l)
      {
        return symbol_type (token::OP_EXISTS, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_OP_FORALL (location_type l)
      {
        return symbol_type (token::OP_FORALL, std::move (l));
      }
#else
      static
      symbol_type
      make_OP_FORALL (const location_type& l)
      {
        return symbol_type (token::OP_FORALL, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
  ...
  private:
    ...
    enum
    {
      yylast_ = 2478,     ///< Last index in yytable_.
