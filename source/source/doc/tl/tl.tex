...
\newcommand{\COLON}{\mathbin{\texttt{:}}}
\newcommand{\COMMA}{\mathbin{\texttt{,}}}
...
\DeclareMathOperator{\EXISTS}{\texttt{\textbackslash{}exists}}
\DeclareMathOperator{\FORALL}{\texttt{\textbackslash{}forall}}
...
\item \label{rule:ap3} Any sequence of alphanumeric characters that
  starts with \samp{F}, \samp{G}, or \samp{X}, has a digit in second
  position, and anything afterwards, is also an atomic proposition,
...
Some of rewritings taken from the syntax of TLSF~\citep{jacobs.16.synt}
...
\begin{align*}
  ...
  f\DELAYR{..} g                & \equiv f\DELAYR{0..} g                  & \DELAYR{..} g         & \equiv \1\DELAYR{0..} g
\end{align*}
...
Spot has some support for quantified formulas.  Quantifiers my only
appear in front of a formulas.  Multiple atomic propositions may be
quantified at once.

\begin{center}
\begin{tabular}{clll}
              & \multicolumn{1}{c}{preferred} & \multicolumn{1}{c}{other supported} & \multicolumn{1}{c}{UTF8}\\
   operation  & \multicolumn{1}{c}{syntax}    & \multicolumn{1}{c}{syntaxes} & \multicolumn{1}{c}{alternative}\\
  \midrule
   existential quantification & $\EXISTS a \COLON f$ && $\exists$ \uni{2203}\\
              & $\EXISTS a_1\COMMA a_2\COMMA\ldots\COMMA a_n \COLON f$ & $\EXISTS a_1\, a_2\, \ldots\, a_n \COLON f$ \\
  universal quantification & $\FORALL a \COLON f$ && $\forall$ \uni{2200}\\
              & $\FORALL a_1\COMMA a_2\COMMA\ldots\COMMA a_n \COLON f$ & $\FORALL a_1\, a_2\, \ldots\, a_n \COLON f$\\
\end{tabular}
\end{center}

\subsection{Semantics}

For an infinite sequence $\sigma\in (\B^\AP)^\omega$, and
$X\subseteq\AP$, let $\sigma_{\setminus X}\in (\B^{\AP\setminus X})^\omega$
designate a sequence such that for all $a\in\AP\setminus X$ and all
positions $i$, the two sequence agree: $\sigma(i)(a) = \sigma_{\setminus X}(i)(a)$.


In the following semantics, we assume that $a$, $a_1$, $a_2$, \ldots
$a_n$ are atomic propositions, and that $f$ is any PSL formula where
these those atomic proposition may be occurring.


\begin{align*}
  \sigma\vDash \EXISTS a \COLON f &\iff \exists \sigma', (\sigma'=\sigma_{\setminus\{a\}})\land(\sigma' \vDash f) \\
  \sigma\vDash \EXISTS a_1\COMMA\,a_2\COMMA\,\ldots,\,a_n \COLON f &\iff \exists \sigma', (\sigma'=\sigma_{\setminus\{a_1,a_2,\ldots, a_n\}})\land(\sigma' \vDash f) \\
  \sigma\vDash \FORALL a \COLON f &\iff \forall \sigma', (\sigma'=\sigma_{\setminus\{a\}})\land(\sigma' \vDash f) \\
  \sigma\vDash \FORALL a_1\COMMA\,a_2\COMMA\,\ldots\COMMA\,a_n \COLON f &\iff \forall \sigma', (\sigma'=\sigma_{\setminus\{a_1,a_2,\ldots, a_n\}})\land(\sigma' \vDash f) \\
\end{align*}

\subsection{Trivial Identities (Occur Automatically)}\label{sec:quant-trivial}

Adjacent quantifiers of the same type are automatically fused,
removing duplicate variables if any.

\begin{align*}
  \EXISTS a_1 \COLON \EXISTS a_2 \COLON f &\equiv \EXISTS a_1\COMMA\, a_2 \COLON f \\
  \EXISTS \underbrace{a_1\COMMA\,a_2\COMMA\,\ldots}_A \COLON \EXISTS \underbrace{b_1\COMMA\,\ldots}_B \COLON f &\equiv
  \EXISTS \underbrace{a_1\COMMA\,a_2\COMMA\,\ldots\COMMA\,b_1\COMMA\,\ldots}_{A \cup B} \COLON f \\
  \FORALL a_1 \COLON \FORALL a_2 \COLON f &\equiv \FORALL a_1\COMMA\, a_2 \COLON f \\
  \FORALL \underbrace{a_1\COMMA\,a_2\COMMA\,\ldots}_A \COLON \FORALL \underbrace{b_1\COMMA\,\ldots}_B \COLON f &\equiv
  \FORALL \underbrace{a_1\COMMA\,a_2\COMMA\,\ldots\COMMA\,b_1\COMMA\,\ldots}_{A \cup B} \COLON f \\
\end{align*}

Additionally, and even if the parser does not support this, building a
quantification over an empty set of variables will simply drop the
quantifier.  (This comes handy when implementing some syntactic
optimizations.)
...
\mathit{atomic\_prop} ::={} & \text{see section~\ref{sec:ap}}                                                                                                                                                                                     \\[1ex]
...
\mathit{sere} ::={}         & \mathit{bformula}                                      & \mid{} & \msamp{\STAR{\mvar{i}..\mvar{j}}}                          & \mid{} & \DELAY{\mvar{i}}\mathit{sere}                                            \\
 ...
 \mid{}                     & \mathit{tformula}\,\msamp{\R}\,\mathit{tformula}                                                                                                                                                                 \\
  ...
  \\
 \mathit{aplist}   ::={}    & \mathit{atomic\_prop} & \mid{} & \mathit{aplist}\,\COMMA\,\mathit{atomic\_prop}
  \\
 \mathit{qformula} ::={}    & \mathit{tformula}                                      & \mid{} & \EXISTS \mathit{aplist} \COLON \mathit{qformula} & \mid{} & \FORALL \mathit{aplist} \COLON \mathit{qformula}
\end{align*}
...
list, the rules use the initial $G$, $S$, $O$, $P$, $R$ of their
...
\begin{tabular}{cl}
  \toprule
  $f\in \Delta_0$, $f\in B$ & \texttt{f.is\_syntactic\_safety() \&\& f.is\_syntactic\_guarantee()} \\
...
\begin{align*}
  ...
  (f_1 \R f_2)\OR  (f_3 \M f_2)&\equiv (f_1\OR f_3) \R f_2\\
  (f_1 \M f_2)\AND (f_1 \M f_3)&\equiv f_1\M (f_2\AND f_3)&
  (f_1 \M f_2)\OR  (f_3 \M f_2)&\equiv (f_1\OR f_3) \M f_2\\
...
\subsubsection{Basic simplifications for quantified formulas}

Let $\varphi$ be a temporal formula, let $\varphi'$ be its negative
normal form constructed as in section Section~\ref{sec:nnf}, and let
$a\in \AP$ be a proposition.

We define the polarity of $a$ in $\varphi$ as:
\begin{description}
\item[missing] if $a$ does not appear in $\varphi$, or if it is already
  quantified in $\varphi$
\item[positive] if $a$ appears in $\varphi$ without being quantified,
  and never under a negation,
\item[negative] if $a$ appears in $\varphi$ without being quantified,
  and always under a negation,
\item[both] if $a$ appears in $\varphi$ without being quantified, both
  under a negation and not under a negation.
\end{description}

For instance in the formula $(a \IMPLIES b) \AND (c \XOR d)$, the
atomic proposition $a$ has negative polarity, $b$ has positive polarity,
$c$ and $d$ have both polarities, and $e$ has missing polarity.

In what follow we assume that $f(m,p,n,b)$ is a temporal formulas over
atomic propositions $m$, $p$, $n$, and $b$, and that this atomic
propositions respectively have missing, positive, negative, and both
polarity.

The following basic simplifications are preformed:
\begin{align*}
  \EXISTS m\COMMA\,p\COMMA\,n\COMMA\,b \COLON f(m,p,n,b) &\equiv \EXISTS b \COLON f(m,\1,\0,b) \\
  \FORALL m\COMMA\,p\COMMA\,n\COMMA\,b \COLON f(m,p,n,b) &\equiv \FORALL b \COLON f(m,\0,\1,b)
\end{align*}
In other words, atomic proposition that are either positive or negative can be replaced by a constant,
missing atomic proposition can be dropped from the quantification list which should only retain atomic
proposition with mixed polarity.

Removing quantified atomic propositions like this might empty set of quantified propositions;
in that case the quantified is simply dropped, as mentioned in Section~\ref{sec:quant-trivial}.


For instance  $\FORALL a\COMMA e\COLON \EXISTS b, c\COLON (a \IMPLIES b) \AND (c \XOR d)$
would be simplified to $\EXISTS c\COLON c \XOR d$.
...
\begin{align*}
  ...
  \F(f_1\OR\ldots\OR f_n \OR \G (g_1 \OR \ldots \OR g_p \OR \X u_1 \OR \ldots \OR \X u_m))&\equiv \F(f_1\OR\ldots\OR f_n \OR \G(g_1 \OR \ldots \OR g_p) \OR u_1 \OR \ldots \OR u_m) \\
