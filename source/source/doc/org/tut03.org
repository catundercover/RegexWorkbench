...
traversing and transforming formulas, and also discuss the
...
second)=, while n-ary operators can additionally take a vector or an initializer list as argument
...
#+BEGIN_SRC C++ :exports code
  ...
  // n-ary operators, in their binary versions
  formula::Or(left, right);       // omega-rational Or
  formula::OrRat(left, right);    // rational Or (for regular expressions)
  formula::And(left, right);      // omega-rational And
  formula::AndRat(left, right);   // rational And (for regular expressions)
  formula::AndNLM(left, right);   // non-length-matching rational And (for r.e.)
  formula::Concat(left, right);   // concatenation (for regular expressions)
  formula::Fusion(left, right);   // concatenation (for regular expressions)
  // star-like operators
...
The binary versions of n-ary operators were introduced in Spot 2.15.
They should be preferred when only two operands are used.
...
Starting with Spot 2.15, each atomic property is now associated to a
unique identifier.  These 0-based identifiers are assigned
incrementally in the order of creation of the atomic properties.  One
use-case for these identifiers is to be able to represent sets of
atomic propositions compactly, as bit vectors.

On a formula that represents an atomic proposition, the =apid()=
method of a formula allows to retrieve the associated identifier.
=apid_count()= static method returns the number of allocated
propositions, for instance so that we can allocate an array to store
information about all propositions.  This count will of course be
adjusted when new atomic proposition are created, and possibly also
when the last use of an atomic-proposition is destroyed.  However
atomic propositions are never renumbered during their lifetime.

The =ap_from_apid()= static method returns the formula representing
the atomic proposition associated to a given ID, and the =apid_map()=
static method /constructs/ and array listing the atomic proposition
for each identifier.
...
#+BEGIN_SRC python
  ...
  print(f"{b.apid()=}")
  print(f"{c.apid()=}")
  print(f"{spot.formula.apid_count()=}")
  print(f"{spot.formula.apid_map()=}")
  del a; print("del a")
  print(f"{spot.formula.apid_count()=}")
  del c; print("del c")
  print(f"{spot.formula.apid_count()=}")
  print(f"{spot.formula.apid_map()=}")
  print(f"{spot.formula.ap_from_apid(1)=}")
  del b; print("del b")
  print(f"{spot.formula.apid_count()=}")
#+END_SRC
...
b.apid()=1
c.apid()=2
spot.formula.apid_count()=3
spot.formula.apid_map()=[spot.formula("a"), spot.formula("b"), spot.formula("c")]
del a
spot.formula.apid_count()=3
del c
spot.formula.apid_count()=2
spot.formula.apid_map()=[None, spot.formula("b")]
spot.formula.ap_from_apid(1)=spot.formula("b")
del b
spot.formula.apid_count()=0
#+end_example
...
#+BEGIN_SRC C++
  ...
  std::ostream& print_vec(const std::vector<spot::formula>& m)
  {
    const char* sep = "[";
    for (spot::formula f: m)
      {
        std::cout << sep;
        if (f)
          std::cout << f;
  ...
  int main()
  {
    ...
    spot::formula b = spot::formula::ap("b");
    spot::formula c = spot::formula::ap("c");
    std::cout << "a.apid()=" << a.apid() << '\n';
    std::cout << "b.apid()=" << b.apid() << '\n';
    std::cout << "c.apid()=" << c.apid() << '\n';
    std::cout << "spot::formula::apid_count()="
              << spot::formula::apid_count() << '\n';
    std::cout << "spot::formula::apid_map()=";
    print_vec(spot::formula::apid_map()) << '\n';
    a = nullptr; std::cerr << "a = nullptr\n";
    std::cout << "spot::formula::apid_count()="
              << spot::formula::apid_count() << '\n';
    c = nullptr; std::cerr << "c = nullptr\n";
    std::cout << "spot::formula::apid_count()="
              << spot::formula::apid_count() << '\n';
    std::cout << "spot::formula::apid_map()=";
    print_vec(spot::formula::apid_map()) << '\n';
    std::cout << "spot::formula::ap_from_apid(1)="
              << spot::formula::ap_from_apid(1) << '\n';
    b = nullptr; std::cerr << "b = nullptr\n";
    std::cout << "spot::formula::apid_count()="
              << spot::formula::apid_count() << '\n';
    return 0;
  }
...
c.apid()=2
spot::formula::apid_count()=3
spot::formula::apid_map()=[a, b, c]
spot::formula::apid_count()=3
spot::formula::apid_count()=2
spot::formula::apid_map()=[nullptr, b]
spot::formula::ap_from_apid(1)=b
spot::formula::apid_count()=0
#+end_example
